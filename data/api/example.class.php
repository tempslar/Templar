...
Class Data_Api_Example extends Common_Data {
	 ...
	 * @return boolean
	 */
	static public function UserClassCheck( $uid, $cid ) {
		if ( $uid  &&  $cid ) {
			$model      = new Model_Api;
			$model->uid = $uid;
			$model->cid = $cid;
	 ...
	 * @return boolean
	 */
	static public function UserClassComplete( $uid, $cid, $agency=1 ) {
		if ( $uid  &&  $cid ) {
			$ucModel = new Model_Api();
			$ucModel->uid = $uid;
			$ucModel->cid = $cid;
			
			$ucModel = Storage_Api_Userclass::GetInst( $ucModel )->getData( array(), FALSE );
	 ...
	 */
	static public function GetData( $uid, $cid ) {
		$outArr = NULL;
	 ...
	 */
	static public function AddData( $uid, $cid, $datas = '' ) {
		$output = FALSE;
	 ...
	 */
	static public function MultiAdd( $uid, $cidStr, $datas = '' ) {
		$output = FALSE;
		$cids   = array();
	 ...
	 * @param int $cid
	 */
	static public function StartClass( $uid, $cid ) {
		...
		if ( isset( $dataArr[ 0 ] ) ) {
			...
			//提交第一次学习时间
				...
				$model = Storage_Api_Userclass::GetInst( $model )->updateData( array(), array( 'uid', 'cid', ) );
	 ...
	 * @return boolean
	 */
	static public function UnlockClass( $uid, $cid, $datas = '' ) {
		$output = FALSE;
		...
		if ( $uid  &&  $cid ) {
			...
			$model = Storage_Api_Userclass::GetInst( $model )
						->updateData( array(), array( 'uid', 'cid' ) );
	 ...
	 * @return boolean
	 */
	static public function LockClass( $uid, $cid ) {
		$output = FALSE;
		...
		if ( $uid && $cid ) {
			...
			$output = Storage_Api_Userclass::GetInst( $model )
						->updateData( array(), array( 'uid', 'cid' ) )->_dataArr;
		}
